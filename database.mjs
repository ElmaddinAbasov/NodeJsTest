...
export class databaseHandler
{
	...
	async #init()
	{
		...
		{
			console.error('ERROR: async #init() failed to connect to a default database - postgres : ', error, client.database);
		}
	}
	...
	async #runQueryWithParams(query, value)
	{
		...
		try
		{
			promise = await this.#client.query(query, value);
		}
		catch (error)
		{
	...
	constructor()
	{
        ...
        async insertIntoTable(query)
        {
                await this.#runQueryWithParams(query.q, query.attribute);
                //await this.#close();
        }
	...
	async setup(clientInfo1, clientInfo2, info)
	{
		...
		this.insertIntoTable({ q : "INSERT INTO " + this.#tableName + "(last_name, first_name, paternal_name,"
